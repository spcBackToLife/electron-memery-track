...
var MemoryCollector = class extends import_events.EventEmitter {
  ...
  /** 采集所有进程信息 */
  collectProcesses() {
    const metrics = import_electron.app.getAppMetrics();
    const wcList = import_electron.webContents.getAllWebContents();
...
var AnomalyDetector = class extends import_events2.EventEmitter {
  constructor(config) {
    super();
    this.snapshots = [];
    this.timer = null;
